...
DEFAULT_DATA_DIR = REPO_ROOT / "results"
DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs/assets/figures"
DEFAULT_MANIFEST = REPO_ROOT / "docs/assets/figures/INSTALL_MANIFEST.md"
...
def render_even_order_milestones(
  order_22: dict,
  order_24: dict,
  output_dir: Path,
) -> list[Path]:
  """Render the order-22 seed beside all nine order-24 survivors."""
  fig = plt.figure(figsize=(10.8, 7.0), dpi=150)
  ...
  seed = order_22["survivors"][0]
  seed_ax = fig.add_subplot(grid[:, 0])
  draw_graph(seed_ax, seed["edges"], 22, r"$H_{22}$: the unique order-$22$ survivor")

  for idx, survivor in enumerate(order_24["survivors"], start=1):
    row, col = divmod(idx - 1, 3)
    ax = fig.add_subplot(grid[row, col + 1])
    graph = graph_from_edges(survivor["edges"], 24)
    girth = graph_girth(graph)
    if idx == 5:
      title = rf"$G_{{{idx}}}$: primitive, $g={girth}$"
      ax.set_facecolor("#fff5dc")
    else:
      title = rf"$G_{{{idx}}}$: blowup, $g={girth}$"
    draw_graph(ax, survivor["edges"], 24, title)

  fig.suptitle(
    "Even-order milestones: one survivor at order 22 and nine at order 24",
    fontsize=13,
    fontweight="bold",
    color=TITLE_COLOR,
    y=0.985,
  ...
  fig.text(
    0.50,
    0.018,
    "Orange vertices have degree 2.  Eight order-24 graphs deflate to "
    r"$H_{22}$; the highlighted $G_5$ is triangle-free.",
    ha="center",
    va="bottom",
    fontsize=8.5,
    color="#333333",
  ...
  fig.patch.set_facecolor("white")
  fig.subplots_adjust(left=0.025, right=0.99, top=0.925, bottom=0.07)

  paths = [
    output_dir / "even_order_22_24_gallery.png",
    output_dir / "even_order_22_24_gallery.pdf",
  ]
...
def write_manifest(manifest_path: Path) -> None:
  content = """# Figure Manifest

All files are generated by `code/scripts/render_graphs.py` from the census
...
- `gallery_order_24.png` -> `docs/assets/figures/gallery_order_24.png`
- `even_order_22_24_gallery.png` -> `docs/assets/figures/even_order_22_24_gallery.png`
- `even_order_22_24_gallery.pdf` -> manuscript figure asset
"""
...
def verify_pngs(paths: list[Path]) -> list[tuple[str, tuple[int, int], float]]:
  ...
  for path in (candidate for candidate in paths if candidate.suffix.lower() == ".png"):
...
def render_all(data_dir: Path, output_dir: Path, manifest_path: Path) -> list[Path]:
  orders_needed = [9, 11, 13, 15, 17, 19, 21, 22, 23, 24]
  ...
  paths.append(render_gallery(24, data_by_order[24], output_dir, limit=None))
  paths.extend(render_even_order_milestones(data_by_order[22], data_by_order[24], output_dir))
